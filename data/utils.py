import numpy as np
import torch
# from .increaseimg import bilinear_interpolation1
import cv2
class Crop(object):
    """
...
class Flip(object):
    ...
    def __call__(self, sample):
        ...
        if flag_lr == 1:
            ...
            # sample['label_128'] = np.fliplr(sample['label_128'])
            # sample['label_64'] = np.fliplr(sample['label_64'])
        if flag_ud == 1:
            sample['image'] = np.flipud(sample['image'])
            sample['label'] = np.flipud(sample['label'])
            # sample['label_128'] = np.flipud(sample['label_128'])
            # sample['label_64'] = np.flipud(sample['label_64'])
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        # image, label,label_128,label_64 = sample['image'], sample['label'], sample['label_128'], sample['label_64']
        image, label= sample['image'], sample['label']
        ...
        # label_128 = np.ascontiguousarray(label_128.transpose((2, 0, 1))[np.newaxis, :])
        # label_64 = np.ascontiguousarray(label_64.transpose((2, 0, 1))[np.newaxis, :])
        sample['image'] = torch.from_numpy(image).float()
        sample['label'] = torch.from_numpy(label).float()
        # sample['label_128'] = torch.from_numpy(label_128).float()
        # sample['label_64'] = torch.from_numpy(label_64).float()
        return sample
