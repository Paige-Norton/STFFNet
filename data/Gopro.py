...
import sys
# sys.path.append('/home/ubuntu/ESTRNN_Trans/data')
# sys.path.append('/home/ubuntu/ESTRNN_Trans')
...
class DeblurDataset(Dataset):
    ...
    def _generate_samples(self, dataset_path, data_format):
        ...
        seqs = sorted(os.listdir(dataset_path), key=str)
        for seq in seqs:
            img_name = os.listdir(join(dataset_path, seq, "blur"))
            label_name = os.listdir(join(dataset_path, seq, "sharp"))
            img_name.sort()
            label_name.sort()
            records[seq] = list()
            for frame in range(len(img_name)):
                suffix = 'png' if data_format == 'RGB' else 'tiff'
                sample = dict()
                sample['Blur'] = join(dataset_path, seq, 'blur', img_name[frame] )
                sample['Sharp'] = join(dataset_path, seq, 'sharp', label_name[frame])
