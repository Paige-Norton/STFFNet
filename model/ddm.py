...
from model.mods import HFRM
from math import sqrt
import torch.nn as nn
from torch.nn import functional as F
import torch.optim
# import clip_loss as clip_loss
import model.clip_score as clip_loss
from collections import OrderedDict
from torch.utils.tensorboard import SummaryWriter
import clip
from model.Dwt_Fre import DWT, IWT, get_Fre
from tqdm import tqdm

device = "cuda" if torch.cuda.is_available() else "cpu"
print(device)
# load clip
c_model, preprocess = clip.load( "/home/ubuntu/ESTRNN_Trans/pretrain_models/prompt_pretrain/ViT-B-32.pt", device=torch.device("cpu"))  # ViT-B/32
c_model.to(device)
...
class DenoisingDiffusion(object):
    ...
    #     pred_LL,  pred_x, denoise_LL_LL_list = output["pred_LL"], output["pred_x"], output["denoise_LL_LL_list"]
    ...
    def clip_loss(self, args, gt, output):
        ...
        # embedding_prompt = prompt.module.embedding_prompt   # 从 embedding_prompt 中提取文本特征，准备用于后续的图像与文本之间的 /
        #                                                     # 相似性计算。在这里，embedding_prompt 似乎是一个用于嵌入文本的预训练层
        # embedding_prompt.requires_grad = False
        # tokenized_prompts = torch.cat([clip.tokenize(p) for p in ["UHD image".join(["X"]*args.length_prompt)]])
        # text_features = text_encoder(embedding_prompt, tokenized_prompts)


        clip_LLLloss = L_clip_LL(pred_x, gt)
        # cliploss = 16*20*L_clip(pred_x, text_features)
        # clip_MSEloss = 25*L_clip_MSE(pred_x, x, [1.0, 1.0, 1.0, 1.0, 0.5])
        c_loss = clip_LLLloss
        # c_loss = cliploss + 0.9*clip_MSEloss+0.7*clip_LLLloss
